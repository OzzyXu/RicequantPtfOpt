...
def get_market_value(stock_list, start_date = '2017-05-26', end_date = '2017-05-26'):
    ...
    if isinstance(stock_list, str):
        ...
        market_value = []
        p2 = p1.reindex_axis(sorted(p1.columns), axis=1)
        #for i in range(len(stock_list)):
        for i in range(p2.shape[1]):
            market_value.append(p2.iloc[:,i] * s1.iloc[:,i])
