...
def input_validation(order_book_ids, start_date, end_date, asset_type, method, rebalancing_frequency, window, bnds,
    ...
    if (method != 'risk_parity' and method != 'min_variance' and method != 'mean_variance' and method != 'min_TE' and method != 'all'):
        return ('请选择合适的优化算法（method）。')

    if (rebalancing_frequency <= 0 or type(rebalancing_frequency) != int):
    ...
    if (res_options != 'weight' and res_options != 'weight_indicators' and res_options != 'all'):
